...
app = FastAPI(title="AI Firewall Enterprise API", version="3.0.0", lifespan=lifespan)
...
async def health_check():
    return HealthResponse(status="healthy", timestamp=datetime.utcnow(), version="3.0.0", services={
...
async def get_organization(current_user: str = Depends(get_current_user)):
    ...
    if not org:
        org = await supabase_manager.ensure_organization(current_user)
    return org
...
async def invite_member(payload: Dict[str, Any], current_user: str = Depends(auth_manager.require_admin)):
    try:
        member = await supabase_manager.invite_member(
            current_user,
        ...
        print(f"Employee invitation failed: {exc}")
        raise HTTPException(status_code=502, detail="Unable to invite employee")
...
async def create_application(payload: Dict[str, Any], current_user: str = Depends(auth_manager.require_admin)):
    ...
    application = await supabase_manager.create_application(
        current_user,
        name,
        str(payload.get("environment") or "production"),
        str(payload.get("provider") or "openai"),
        str(payload.get("model") or "gpt-4o-mini"),
    )
...
async def create_api_key(payload: Dict[str, Any] | None = None, current_user: str = Depends(auth_manager.require_admin)):
    ...
    expires_days = max(1, min(int(payload.get("expires_days") or 30), 3650))
...
async def run_firewall_pipeline(prompt: str, model: str, max_tokens: int, temperature: float, user_id: str, request_id: str, gateway_context: Dict[str, Any] | None = None):
    ...
    org_id = gateway_context.get("organization_id")
    app_id = gateway_context.get("application_id")
    employee_id = gateway_context.get("employee_id")
    event_base = {"user_id": user_id, "organization_id": org_id, "application_id": app_id, "employee_id": employee_id}
    ...
    sanitized_prompt = await orchestrator.sanitize_prompt(prompt, redact_pii=security_result.pii_detected)
    guarded_prompt = await orchestrator.apply_prompt_guard(sanitized_prompt, context={})
    llm_response = await llm_router.route(prompt=guarded_prompt, model=model, max_tokens=max_tokens, temperature=temperature, request_id=request_id)
    if llm_response.get("error"):
        raise HTTPException(status_code=502, detail="AI provider unavailable")

    filtered_response = await orchestrator.filter_response(llm_response=llm_response, prevent_data_leakage=True, block_harmful_content=True)
    latency_ms = (time.time() - start_time) * 1000
...
async def chat_completions(payload: Dict[str, Any], key_data: Dict = Depends(api_key_manager.verify_api_key_context)):
    """OpenAI-compatible enterprise gateway authenticated by an AI Firewall API key."""
    if key_data.get("organization_id") and key_data.get("application_id"):
        ...
        if not app_row.data or app_row.data.get("status") != "active":
            raise HTTPException(status_code=403, detail="Application is disabled")

    messages = payload.get("messages") or []
    if not isinstance(messages, list) or not messages:
        raise HTTPException(status_code=400, detail="messages must be a non-empty array")
    user_messages = [str(message.get("content", "")) for message in messages if isinstance(message, dict) and message.get("role") == "user"]
    ...
    if result["blocked"]:
        return JSONResponse(status_code=403, content={"error": {"message": "Request blocked by AI Firewall security policy", "type": "firewall_blocked", "code": "security_policy", "risk_score": result["risk_score"], "reason": result["reason"]}, "request_id": request_id})
...
async def secure_query(request: SecurityRequest, background_tasks: BackgroundTasks, current_user: str = Depends(get_current_user), http_request: Request = None):
    request_id = str(uuid.uuid4())
    result = await run_firewall_pipeline(request.prompt, request.model.value if hasattr(request.model, "value") else str(request.model), request.max_tokens or 1000, request.temperature if request.temperature is not None else 0.7, current_user, request_id)
    if result["blocked"]:
        return SecurityResponse(request_id=request_id, allowed=False, blocked=True, message="Request blocked by security policy", risk_score=result["risk_score"], reason=result["reason"], timestamp=datetime.utcnow())
...
# ---------------------------------------------------------------------------
# Existing personal dashboard APIs
# ---------------------------------------------------------------------------
...
async def get_recent_requests(limit: int = 50, current_user: str = Depends(get_current_user)):
    return {"requests": await supabase_manager.get_org_activity(current_user, limit)}
...
async def get_audit_logs(limit: int = 100, offset: int = 0, user_id: str = None, current_user: str = Depends(get_current_user), is_admin: bool = Depends(auth_manager.require_admin)):
