...
async def webhook(request:Request):
    ...
    if event_id and supabase.client.table("billing_events").select("id").eq("stripe_event_id",event_id).maybe_single().execute().data: return {"received":True,"duplicate":True}
    if event_type in {"checkout.session.completed","customer.subscription.created","customer.subscription.updated"}:
        subscription_id=obj.get("subscription") if event_type=="checkout.session.completed" else obj.get("id"); customer_id=obj.get("customer")
        if subscription_id and customer_id:
            items=(obj.get("items") or {}).get("data") or []; item=items[0] if items else {}; quantity=int(item.get("quantity") or 1); price=(item.get("price") or {}); price_id=price.get("id"); plan_key=metadata.get("plan_key") or next((k for k in PLANS if os.getenv(f"STRIPE_PRICE_{k.upper()}")==price_id),"starter")
            if org_id: supabase.client.table("billing_subscriptions").upsert({"organization_id":org_id,"stripe_subscription_id":subscription_id,"stripe_customer_id":customer_id,"plan_key":plan_key,"status":obj.get("status","active"),"seat_quantity":quantity,"current_period_end":datetime.fromtimestamp(obj["current_period_end"],tz=timezone.utc).isoformat() if obj.get("current_period_end") else None,"cancel_at_period_end":bool(obj.get("cancel_at_period_end",False)),"updated_at":datetime.now(timezone.utc).isoformat()},on_conflict="organization_id").execute()
    elif event_type in {"customer.subscription.deleted","invoice.payment_failed","invoice.paid"}:
        customer_id=obj.get("customer")
        if customer_id: supabase.client.table("billing_subscriptions").update({"status":"canceled" if event_type.endswith("deleted") else ("past_due" if event_type=="invoice.payment_failed" else "active"),"updated_at":datetime.now(timezone.utc).isoformat()}).eq("stripe_customer_id",customer_id).execute()
    if event_id: supabase.client.table("billing_events").insert({"stripe_event_id":event_id,"event_type":event_type,"organization_id":org_id,"payload":{"id":event_id,"type":event_type}}).execute()
