...
export const AuthProvider = ({ children }) => {
  ...
  const loadProfile = useCallback(async (currentUser) => { if (!currentUser) { setProfile(null); setAuthPolicy({require_mfa:false,require_sso:false}); setMfaRequired(false); setProfileLoading(false); return null }; setProfileLoading(true); try { const {data,error}=await supabase.from('profiles').select('id,full_name,role,organization_id').eq('id',currentUser.id).maybeSingle(); if(error)throw error; setProfile(data??null); let policyData={require_mfa:false,require_sso:false}; if(data?.organization_id){const policy=await supabase.from('organization_settings').select('require_mfa,require_sso').eq('organization_id',data.organization_id).maybeSingle(); if(!policy.error&&policy.data){policyData=policy.data;setAuthPolicy(policyData)}} return policyData } catch(error){console.error('Failed to load authenticated profile',error);setProfile(null);return {require_mfa:false,require_sso:false}} finally{setProfileLoading(false)} },[])
