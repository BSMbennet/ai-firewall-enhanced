...
export const AuthProvider = ({ children }) => {
  ...
  const loadProfile = useCallback(async (currentUser) => {
    if (!currentUser) { setProfile(null); setAuthPolicy({ require_mfa: false, require_sso: false }); setMfaRequired(false); setProfileLoading(false); return }
    ...
    try {
      ...
      if (data?.organization_id) { const policy = await supabase.from('organization_settings').select('require_mfa,require_sso').eq('organization_id', data.organization_id).maybeSingle(); if (!policy.error && policy.data) setAuthPolicy(policy.data) }
    } catch (error) { console.error('Failed to load authenticated profile', error); setProfile(null) } finally { setProfileLoading(false) }
  }, [])
