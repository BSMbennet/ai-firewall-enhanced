...
export const AuthProvider = ({ children }) => {
  ...
  const loadProfile = useCallback(async (currentUser) => {
    if (!currentUser) {
      setProfile(null); setAuthPolicy({require_mfa:false,require_sso:false}); setMfaRequired(false); setProfileLoading(false); return null
    }
    setProfileLoading(true)
    try {
      let {data,error}=await supabase.from('profiles').select('id,full_name,role,organization_id').eq('id',currentUser.id).maybeSingle()
      if(error)throw error

      // A newly registered workspace creator may have a profile row without an organization yet.
      // Provision the organization through the authenticated backend endpoint; it makes this
      // first account the organization owner, then refresh the profile so RBAC is immediately correct.
      if(data && !data.organization_id){
        try {
          await api.get('/organization')
          const refreshed=await supabase.from('profiles').select('id,full_name,role,organization_id').eq('id',currentUser.id).maybeSingle()
          if(!refreshed.error&&refreshed.data)data=refreshed.data
        } catch(provisionError) {
          console.error('Failed to provision organization for authenticated user',provisionError)
        }
      }
      ...
      if(data?.organization_id){
        ...
        if(!policy.error&&policy.data){policyData=policy.data;setAuthPolicy(policyData)}
      }
      return policyData
    } catch(error){
      console.error('Failed to load authenticated profile',error);setProfile(null);return {require_mfa:false,require_sso:false}
    } finally{setProfileLoading(false)}
  },[])

  const refreshMfaRequirement = useCallback(async()=>{if(!user||!authPolicy.require_mfa){setMfaRequired(false);return};const {data,error}=await supabase.auth.mfa.getAuthenticatorAssuranceLevel();if(!error)setMfaRequired(data?.nextLevel==='aal2'&&data?.currentLevel!=='aal2')},[user,authPolicy.require_mfa])
  useEffect(()=>{let mounted=true;const init=async()=>{try{const code=new URLSearchParams(window.location.search).get('code');if(code){const {error}=await supabase.auth.exchangeCodeForSession(code);if(error)throw error;window.history.replaceState({},document.title,'/dashboard')}const {data,error}=await supabase.auth.getSession();if(!mounted)return;if(error)console.error(error);const next=data.session??null;setSession(next);setUser(next?.user??null);setLoading(false);await loadProfile(next?.user??null)}catch(error){console.error('SSO callback failed',error);if(mounted)setLoading(false)}};init();const {data:subscription}=supabase.auth.onAuthStateChange((_event,next)=>{if(!mounted)return;setSession(next??null);setUser(next?.user??null);setSessionWarning(false);setLoading(false);window.setTimeout(()=>mounted&&loadProfile(next?.user??null),0)});return()=>{mounted=false;subscription.subscription.unsubscribe()}},[loadProfile])
  ...
  return <AuthContext.Provider value={value}>{children}{mfaRequired&&user&&<MFA onVerified={refreshMfaRequirement}/>} {sessionWarning&&user&&!mfaRequired&&<SessionTimeoutModal onStaySignedIn={staySignedIn} onLogout={()=>logoutAndRedirect(signOut)}/>}</AuthContext.Provider>
}
