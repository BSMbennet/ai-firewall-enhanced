...
export const AuthProvider = ({ children }) => {
  ...
  const loadProfile = useCallback(async (currentUser) => {
    if (!currentUser) {
      setProfile(null)
      setAuthPolicy({ require_mfa: false, require_sso: false })
    ...
    try {
      ...
      setProfile(null)
      return { require_mfa: false, require_sso: false }
    } finally {
      setProfileLoading(false)
    }
  }, [])
  ...
  const refreshMfaRequirement = useCallback(async () => {
    if (!user || !authPolicy.require_mfa) {
      setMfaRequired(false)
      return
    }
  ...
  useEffect(() => {
    ...
    const { data: subscription } = supabase.auth.onAuthStateChange((_event, next) => {
      ...
      setLoading(false)
      window.setTimeout(() => mounted && loadProfile(next?.user ?? null), 0)
    })
  ...
  const signIn = async (email, password) => {
    ...
    setUser(data.user)
    const policy = await loadProfile(data.user)
    return { ...data, policy }
  }

  const signInWithSSO = async (email) => {
    const domain = String(email || '').split('@')[1]?.trim().toLowerCase()
    if (!domain) throw new Error('Enter your work email first')
  ...
  return (
    <AuthContext.Provider value={value}>
      {children}
      {mfaRequired && user && <MFA onVerified={refreshMfaRequirement} />}
      {sessionWarning && user && !mfaRequired && (
