...
export default function AppShell({children}){
 ...
 return <div className="af-app">
   <header className="af-header">
    ...
    <div className="af-profile"><span className="af-email">{user?.email||'operator@secure'}</span><span className="af-avatar">{(user?.email||'A')[0].toUpperCase()}</span><button className="af-menu-btn" onClick={()=>setOpen(!open)} aria-label="Open navigation menu">{open?<X size={18}/>:<Menu size={18}/>}</button></div>
   </header>
